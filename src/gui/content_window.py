# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'D:\Projects\HandSight\src\qt_designer_files\content_window.ui'
#
# Created by: PyQt5 UI code generator 5.15.11
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets
import gui.resources_rc

class Ui_main_window(object):
    def setupUi(self, main_window):
        main_window.setObjectName("main_window")
        main_window.resize(1280, 720)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(main_window.sizePolicy().hasHeightForWidth())
        main_window.setSizePolicy(sizePolicy)
        main_window.setMinimumSize(QtCore.QSize(1280, 720))
        main_window.setMaximumSize(QtCore.QSize(1280, 720))
        main_window.setStyleSheet("background-color: rgb(62, 65, 74);")
        self.centralwidget = QtWidgets.QWidget(main_window)
        self.centralwidget.setObjectName("centralwidget")
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(80, 180, 220, 70))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(32)
        font.setBold(True)
        font.setItalic(True)
        font.setWeight(75)
        self.label.setFont(font)
        self.label.setStyleSheet("color: rgb(255, 255, 255);")
        self.label.setObjectName("label")
        self.label_2 = QtWidgets.QLabel(self.centralwidget)
        self.label_2.setGeometry(QtCore.QRect(100, 60, 181, 121))
        self.label_2.setText("")
        self.label_2.setPixmap(QtGui.QPixmap(":/res/main_icon.svg"))
        self.label_2.setScaledContents(True)
        self.label_2.setObjectName("label_2")
        self.splitter = QtWidgets.QSplitter(self.centralwidget)
        self.splitter.setGeometry(QtCore.QRect(140, 570, 90, 90))
        self.splitter.setOrientation(QtCore.Qt.Horizontal)
        self.splitter.setHandleWidth(10)
        self.splitter.setObjectName("splitter")
        self.tg_link = QtWidgets.QLabel(self.splitter)
        self.tg_link.setText("")
        self.tg_link.setPixmap(QtGui.QPixmap(":/res/tg_icon.svg"))
        self.tg_link.setScaledContents(True)
        self.tg_link.setObjectName("tg_link")
        self.actions = QtWidgets.QSplitter(self.centralwidget)
        self.actions.setEnabled(True)
        self.actions.setGeometry(QtCore.QRect(60, 260, 260, 280))
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.actions.sizePolicy().hasHeightForWidth())
        self.actions.setSizePolicy(sizePolicy)
        self.actions.setOrientation(QtCore.Qt.Vertical)
        self.actions.setHandleWidth(15)
        self.actions.setObjectName("actions")
        self.video_stream_button = QtWidgets.QPushButton(self.actions)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.video_stream_button.sizePolicy().hasHeightForWidth())
        self.video_stream_button.setSizePolicy(sizePolicy)
        self.video_stream_button.setMinimumSize(QtCore.QSize(260, 59))
        self.video_stream_button.setMaximumSize(QtCore.QSize(260, 59))
        font = QtGui.QFont()
        font.setFamily("Comic Sans MS")
        font.setPointSize(16)
        self.video_stream_button.setFont(font)
        self.video_stream_button.setStyleSheet("border-radius: 20px;\n"
"                            background-color: rgb(217, 217, 217);\n"
"                        ")
        self.video_stream_button.setObjectName("video_stream_button")
        self.settings_button = QtWidgets.QPushButton(self.actions)
        self.settings_button.setMinimumSize(QtCore.QSize(260, 59))
        self.settings_button.setMaximumSize(QtCore.QSize(260, 59))
        font = QtGui.QFont()
        font.setFamily("Comic Sans MS")
        font.setPointSize(16)
        self.settings_button.setFont(font)
        self.settings_button.setStyleSheet("border-radius: 20px;\n"
"                            background-color: rgb(217, 217, 217);\n"
"                        ")
        self.settings_button.setObjectName("settings_button")
        self.gestures_button = QtWidgets.QPushButton(self.actions)
        self.gestures_button.setEnabled(True)
        self.gestures_button.setMinimumSize(QtCore.QSize(260, 59))
        self.gestures_button.setMaximumSize(QtCore.QSize(260, 59))
        font = QtGui.QFont()
        font.setFamily("Comic Sans MS")
        font.setPointSize(16)
        self.gestures_button.setFont(font)
        self.gestures_button.setStyleSheet("border-radius: 20px;\n"
"                            background-color: rgb(217, 217, 217);\n"
"                        ")
        self.gestures_button.setObjectName("gestures_button")
        self.exit_button = QtWidgets.QPushButton(self.actions)
        self.exit_button.setMinimumSize(QtCore.QSize(260, 59))
        self.exit_button.setMaximumSize(QtCore.QSize(260, 59))
        font = QtGui.QFont()
        font.setFamily("Comic Sans MS")
        font.setPointSize(16)
        self.exit_button.setFont(font)
        self.exit_button.setStyleSheet("border-radius: 20px;\n"
"                            background-color: rgb(217, 217, 217);\n"
"                        ")
        self.exit_button.setObjectName("exit_button")
        self.functions_widget = QtWidgets.QStackedWidget(self.centralwidget)
        self.functions_widget.setGeometry(QtCore.QRect(329, 59, 900, 600))
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.functions_widget.sizePolicy().hasHeightForWidth())
        self.functions_widget.setSizePolicy(sizePolicy)
        self.functions_widget.setStyleSheet("background-color: rgb(217, 217, 217);\n"
"                        border-radius: 20px;\n"
"                    ")
        self.functions_widget.setObjectName("functions_widget")
        main_window.setCentralWidget(self.centralwidget)

        self.retranslateUi(main_window)
        self.functions_widget.setCurrentIndex(-1)
        QtCore.QMetaObject.connectSlotsByName(main_window)

    def retranslateUi(self, main_window):
        _translate = QtCore.QCoreApplication.translate
        main_window.setWindowTitle(_translate("main_window", "HandSight - wanna see your gesture"))
        self.label.setText(_translate("main_window", "HandSight"))
        self.video_stream_button.setText(_translate("main_window", "Трансляция"))
        self.settings_button.setText(_translate("main_window", "Настройки"))
        self.gestures_button.setText(_translate("main_window", "Меню Жестов"))
        self.exit_button.setText(_translate("main_window", "Выйти"))

